...
from convert_mediapipe_index import real_world_landmark_names
import re
    ...
    try:
        ...
        for index, row in df.iterrows():
            ...
            for hvp in hand_view_pose_group:
                ...
                # Append the new item to the ndf dataframe                  #
                second_half_df = pd.DataFrame(columns=second_half_columns,  data=[row.iloc[1:].values.tolist()])
                item_df = pd.DataFrame([item])
                sdf = pd.concat([item_df.reset_index(drop=True), second_half_df.reset_index(drop=True)], axis=1)

                ndf = pd.concat([ndf, sdf], axis=0, ignore_index=True)

                # Save the file to the output file.
